...
const reviewItems = [
  ...
  'Payment status during COVID forbearance, deferment, PSLF, IDR, SAVE, or other protected periods',
  'Duplicate tradelines after a transfer or consolidation',
  'Balances that do not match payoff, discharge, forgiveness, or transfer records',
  'Late payments, date of first delinquency, account status, and bureau-specific reporting differences',
]
...
export default function StudentLoansPage() {
  ...
  const downloadLetter = () => {
    ...
    link.href = url
    link.download = 'settled-student-loan-dispute-letter.txt'
  ...
  const saveToDashboard = async () => {
    ...
    try {
      ...
      id: `${result.legacy_case_type || 'student-loans'}-${Date.now()}`,
      type: result.legacy_case_type || 'student-loans',
      title: makeCaseTitle(result.legacy_case_type || 'student-loans', result.violations || [], result.summary),
      summary: result.summary || 'Student loan scan saved from SETTLED.',
          ...
          <div className="space-y-6">
            <div>
              ...
              <p
                ...
                style={{ fontFamily: 'var(--font-plus-jakarta-sans)' }}
              >
                Best documents: full credit report tradeline, servicer payment history, forbearance notice, transfer
                notice, discharge/forgiveness notice, or bureau investigation response.
              </p>
            </div>
              ...
              {result.response && (
                <div className="space-y-2">
                  ...
                  <div className="rounded-lg border border-amber-400/20 bg-amber-400/10 p-4 text-xs leading-relaxed text-amber-100/85">
                    Review every date, bureau, account number, and fact before sending. Do not send a dispute for a
                    debt or late payment you know is accurate.
                  </div>
